...
if __name__ == "__main__":
    ...
    query_runner = SQLAlchemyQueryRunner(config_file)
    ...
    df = pd.DataFrame(query_runner.get_list_from_query(query), columns=['country', 'total_sales'])
    ...
    if query_runner.run_query(sql_folder / '00_rpt_sales.sql'):
    ...
    result = query_runner.get_list_from_query(sql_folder / '01_create.sql')
    print(f"Number of records created: {len(result)}")
    print('-' * 100)

    print("Reading records from the 'rpt_sales' table")
    print(pd.DataFrame(query_runner.get_dataframe_from_query(sql_folder / '02_read.sql')))
    print('-' * 100)

    print("Updating records in the 'rpt_sales' table - Updating revenue for the year 1997 by increasing it by 10%")
    result = query_runner.get_list_from_query(sql_folder / '03_update.sql')
    ...
    result = query_runner.get_list_from_query(sql_folder / '04_delete.sql')
    print(f"Number of records deleted: {len(result)}")
    print('-' * 100)

    print("Part 3: Dropping the 'rpt_sales' table to clean up the database")
    if query_runner.run_query(sql_folder / '05_drop.sql'):
